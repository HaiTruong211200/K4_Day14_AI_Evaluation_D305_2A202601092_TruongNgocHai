    ...
    def score_response(
        ...
        try:
            ...
            scores = {
                criterion: max(
                    0.0,
                    min(1.0, float(parsed.get(criterion, 0.5)))
                )
...
class BenchmarkRunner:
    ...
    def generate_report(self, results: list[EvalResult]) -> dict[str, Any]:
        ...
        avg_faithfulness = (
            sum(r.faithfulness for r in results) / total if total else 0.0
        )
        avg_relevance = (
            sum(r.relevance for r in results) / total if total else 0.0
        )
        avg_completeness = (
            sum(r.completeness for r in results) / total if total else 0.0
        )
...
class FailureAnalyzer:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for index, failure in enumerate(failures, start=1):
            suggested_fix = (
                ...
                else "Review the failed case and improve the evaluation pipeline"
            )
            lines.append(
                f"| F{index:03d} | "
                f"{markdown_cell(failure.failure_type or 'unknown')} | "
                f"{markdown_cell(self.find_root_cause(failure))} | "
                f"{markdown_cell(suggested_fix)} | Open |"
            )
    ...
    def generate_improvement_suggestions(
        ...
        suggestion_by_type = {
            "hallucination": (
                "Implement a faithfulness guardrail to reject claims unsupported "
                "by retrieved context"
            ),
            "irrelevant": (
                "Improve prompt instructions and intent detection so answers "
                "address the user's question"
            ),
            "incomplete": (
                "Improve retrieval coverage and add few-shot examples of complete "
                "answers"
            ),
            "off_topic": (
                "Add query classification and clearer scope instructions to keep "
                "answers on topic"
            ),
            "refusal": (
                "Review safety guardrails to reduce unnecessary refusals for "
                "in-scope questions"
            ),
        }
        ...
        for failure_type, _ in sorted(
            ...
            suggestion = suggestion_by_type.get(
                normalized_type,
                f"Review {failure_type} failures and add targeted regression cases",
            )
            if suggestion not in suggestions:
                suggestions.append(suggestion)

        fallback_suggestions = [
            "Inspect low-scoring traces and add representative cases to the golden dataset",
            "Tune retrieval chunking and ranking using Context Recall and Context Precision",
            "Add automated regression checks before deploying prompt or model changes",
        ]
